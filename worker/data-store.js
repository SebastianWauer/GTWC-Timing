...
export class DataStore {
  ...
  applyEventList({ passes, flags, messages, isTotal }) {
    ...
    for (const p of passes) {
      ...
      if (st === 'I1' && p.it !== null) {
        ...
        ls.s1 = p.it; ls.s2 = null; ls.s3 = null;
        if (p.dr) {
          ls.drId = p.dr;
          if (car) {
            const entry = this._entryMap.get(nr);
  ...
  _emptyCarRecord(nr) {
    return {
      ...
      _bestLapByDriver: {},
      _pitEntryTimeMs: null,
      _stintStartTimeMs: null,
      _lastPitDurationMs: null,
    };
  }
  ...
  getSnapshot(classFilter = null) {
    ...
    return {
      ...
      bestSectors: this._getSessionBestSectors(classFilter),
      sessionBestSectors: this._getSessionBestSectors(null),
